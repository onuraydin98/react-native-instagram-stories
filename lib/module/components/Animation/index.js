...
const StoryAnimation = ({ children, x, index }) => {
    ...
    const animatedStyle = useAnimatedStyle(() => {
        ...
        const left = Platform.OS === 'android' ? interpolate(rotateY, [-angle, -angle + 0.1, 0, angle - 0.1, angle], [0, 20, 0, -20, 0], Extrapolation.CLAMP) : 0;
        return {
            transform: [
                { perspective: WIDTH },
                { translateX },
    ...
    return (<Animated.View style={[animatedStyle, AnimationStyles.container, AnimationStyles.cube]}>
      {children}
