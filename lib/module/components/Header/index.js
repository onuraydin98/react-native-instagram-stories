...
const StoryHeader = ({ avatarSource, name, onClose, avatarSize, textStyle, closeColor, headerStyle, headerContainerStyle, renderStoryHeader, onStoryHeaderPress, }) => {
    ...
    if (renderStoryHeader) {
        return (<View style={[HeaderStyles.container, { width }, headerContainerStyle]}>
        {renderStoryHeader()}
      </View>);
    }
    return (<View style={[
      ...
      <Pressable style={[HeaderStyles.left, headerStyle]} onPress={() => onStoryHeaderPress?.()}>
        {Boolean(avatarSource) && (<LinearGradient colors={["#FCC94F", "#A80533"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{
                padding: 2,
                borderRadius: 100,
            }}>
            <View style={[
