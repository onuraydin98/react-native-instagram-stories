...
const StoryImage = ({ stories, activeStory, defaultStory, isDefaultVideo, paused, videoProps, isActive, mediaContainerStyle, imageStyles, imageProps, videoDuration, onImageLayout, onLoad, }) => {
    const [data, setData] = useState({ data: defaultStory, isVideo: isDefaultVideo });
    ...
    const onImageChange = async () => {
        ...
        if (nextStory && nextStory.mediaType !== 'video' && nextStory.source?.uri) {
            Image.prefetch(nextStory.source?.uri);
        }
    };
    useAnimatedReaction(() => isActive.value, (res, prev) => res !== prev && res && runOnJS(onImageChange)(), [isActive.value, onImageChange]);
    useAnimatedReaction(() => activeStory.value, (res, prev) => res !== prev && runOnJS(onImageChange)(), [activeStory.value, onImageChange]);
    ...
    return (<>
      <View style={ImageStyles.container}>
        <Loader loading={loading} color={color} size={80}/>
      </View>
      <View style={[ImageStyles.image, mediaContainerStyle]}>
        {data.data?.source && (data.isVideo ? (<StoryVideo onLoad={onContentLoad} onLayout={onImageLayout} source={data.data.source} paused={isPaused} isActive={isActive} {...videoProps}/>) : (<Image source={data.data.source} style={[{ width: WIDTH, aspectRatio: 0.5626 }, imageStyles]} resizeMode="contain" testID="storyImageComponent" onLayout={(e) => onImageLayout(Math.min(HEIGHT, e.nativeEvent.layout.height))} onLoad={() => onContentLoad()} {...imageProps}/>))}
      </View>
    </>);
};
